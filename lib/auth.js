...
function verifyAdmin(username, password) {
  const configuredUsers = parseAdminUsers();
  if (configuredUsers) {
    const expectedPass = configuredUsers[String(username || "")];
    return comparePassword(password, expectedPass);
  }
...
function comparePassword(password, expectedPass) {
  if (!expectedPass) return false;
  const a = Buffer.from(String(password ?? ""), "utf8");
  const b = Buffer.from(String(expectedPass), "utf8");
